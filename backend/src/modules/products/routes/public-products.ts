...
import { offerSchema, productSchema, tagSchema, categorySchema, errorResponseSchema } from "../schemas/productSchemas.js";

export async function registerPublicProductRoutes(app: FastifyInstance) {
  ...
  app.get("/api/products/:slug", {
    ...
    handler: async (request, reply) => {
      ...
      const versions = await app.cache.getVersions(["products", "categories", "tags", "offers"]);
      const cacheKey = buildCacheKey("cache:product", {
        slug: params.slug,
        vProducts: versions.products,
        vCategories: versions.categories,
        vTags: versions.tags,
